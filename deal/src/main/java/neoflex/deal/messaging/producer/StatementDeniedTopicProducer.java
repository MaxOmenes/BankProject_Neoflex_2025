...
public class StatementDeniedTopicProducer implements TopicProducer<EmailMessageDto> {

    private final KafkaTemplate<String, Object> kafkaTemplate;

    @Override
    public void send(EmailMessageDto message) {
        kafkaTemplate.send(Topic.STATEMENT_DENIED.getName(), message.getStatementId().toString(), message);
