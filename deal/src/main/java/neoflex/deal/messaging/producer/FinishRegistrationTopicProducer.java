...
public class FinishRegistrationTopicProducer implements TopicProducer<EmailMessageDto> {
    private final KafkaTemplate<String, Object> kafkaTemplate;
    @Override
    public void send(EmailMessageDto message) {
        kafkaTemplate.send(Topic.FINISH_REGISTRATION.getName(), message.getStatementId().toString(), message);
    }
}
