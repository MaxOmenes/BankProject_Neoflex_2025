...
public class StatementDeniedTopicHandlerService implements TopicHandler<EmailMessageDto> {
    ...
    public void handle(EmailMessageDto message) {
        ...
        String emailContent = templateEngine.process(Templates.STATEMENT_DENIED.getPath(), context);

        emailSenderService.sendEmail(message.getAddress(), "Statement denied | Bank with Love", emailContent);
    }
}
