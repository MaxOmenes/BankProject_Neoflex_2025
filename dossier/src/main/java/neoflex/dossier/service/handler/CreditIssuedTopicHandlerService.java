...
public class CreditIssuedTopicHandlerService implements TopicHandler<EmailMessageDto> {
    ...
    public void handle(EmailMessageDto message) {
        ...
        String emailContent = templateEngine.process(Templates.CREDIT_ISSUED.getPath(), context);

        emailSenderService.sendEmail(message.getAddress(), "Your credit issued | Bank with Love" , emailContent);
    }
}
